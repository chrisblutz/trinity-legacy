 ...
 */
public class ValInterpreter extends DeclarationInterpreter {
    ...
    public void interpret(Line line, Block nextBlock, InterpretEnvironment env, String fileName, File fullFile) {
        ...
        if (line.size() >= 2) {
            ...
            if (line.get(0).getToken() == Token.VAL) {
                ...
                if (!env.getClassStack().isEmpty()) {
                    ...
                    Errors.throwSyntaxError("Trinity.Errors.ScopeError", "Constants must be declared within a class.", fileName, line.getLineNumber());
                }
